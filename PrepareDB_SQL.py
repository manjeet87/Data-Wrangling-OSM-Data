...
OSM_PATH1 = "example3.osm"
OSM_PATH = r"G:\Manjeet\Nano degree\Module 4_ Data Wrangling\Santa_Rose_map.osm"
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for tag in element.iter('tag'):
            nodetag = {}
            nodetag['id'] = int(node_attribs['id'])


            if ':' in tag.attrib['k']:
                address = tag.attrib['k'].split(':')
                nodetag['type'] =  address[0]
                nodetag['key'] = ':'.join(x for x in address[1:])
            else:
                nodetag['type'] = 'regular'
                nodetag['key'] = tag.attrib['k']

            nodetag['value'] = tag.attrib['v']
        ...
        for node in element.iter('nd'):
            waynode = {}
            waynode['id'] = int(way_attribs['id'])
            waynode['node_id'] = int(node.attrib['ref'])
            waynode['position'] = int(node_pos)
            node_pos +=1
        ...
        for way_tag in element.iter('tag'):
            waytag = {}
            waytag['id'] = int(waynode['id'])

            if ':' in way_tag.attrib['k']:
                address = way_tag.attrib['k'].split(':')
                waytag['type'] = address[0]
                waytag['key'] = ':'.join(x for x in address[1:])
            else:
                waytag['type'] = 'regular'
                waytag['key'] = way_tag.attrib['k']

            waytag['value'] = way_tag.attrib['v']
            tags.append(waytag)
